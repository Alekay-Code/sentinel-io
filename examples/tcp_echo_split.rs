...
fn main() {
    sentinel::block_on(async {
        let server = sentinel::spawn(async {
            ...
            let reader_handler = sentinel_io::spawn(async move {
                let amount = reader.read(&mut buffer).await.unwrap();
                let msg = String::from_utf8_lossy(&buffer[0..amount]);
                println!("[SERVER]: Read {amount} bytes: '{msg}'");
            });

            let writer_handler = sentinel_io::spawn(async move {
                println!("[SERVER]: Sending '{msg}' to {}", addr);
                let _ = writer.write(msg.as_bytes()).await;
            });

            reader_handler.await;
